...
import { SearchProduct, AddPackage } from '../../Redux/Action'
import { Redirect } from 'react-router-dom'

const AddPackagePage = (props) => {
	...
	const handleChange = (e) => {
		...
		setSelectedProduct((prevState) => {
			...
			update[e.target.name].qty = e.target.value
			return [...update]
		})
	}
			...
			return (
				<div className='container'>
					<div className='row justify-content-center mt-3'>
						...
						<div className='col-12'>
							<h6 className='text-center'>Please try another search</h6>
						</div>
					</div>
				</div>
			)
	...
	if (redirect) return <Redirect to='/admin/manage-product/package?success=1' />
	return (
		<div>
					...
					{selectedProduct.map((prod, index) => (
						<div key={index} className='mb-2'>
							...
							<div style={{ width: '20%' }}>
								...
								<input
									value={selectedProduct[index].qty ? selectedProduct[index].qty : ''}
				...
				{search ? (
					loading ? (
						...
						<table className='table'>
							<tbody>{productData && renderProductData()}</tbody>
						</table>
					)
				) : (
